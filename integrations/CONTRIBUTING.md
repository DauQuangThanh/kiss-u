...
Built-in integrations are maintained by the kiss core team and ship with the CLI.

### Checklist

1. **Create the integration subpackage** under `src/kiss_cli/integrations/<package_dir>/`
...
3. **Register the integration** in `src/kiss_cli/integrations/__init__.py`
...
1. **Working integration** — tested with `kiss integration install`
...
integration:
  ...
  description: "Integration for My Agent"
  author: "your-name"
  repository: "https://github.com/your-name/kiss-my-agent"
  license: "MIT"
requires:
  kiss_version: ">=0.6.0"
  tools:
...
provides:
  commands:
    - name: "kiss.specify"
      file: "templates/kiss.specify.md"
  scripts:
    - update-context.sh
```
...
| `requires.kiss_version` | Required field; specify a version constraint such as `>=0.6.0` (current validation checks presence only) |
...
1. **Fork** the [kiss repository](https://github.com/DauQuangThanh/kiss-u)
...
The `kiss integration upgrade` command supports diff-aware upgrades:
...
kiss integration upgrade

# Force upgrade (overwrites modified files)
kiss integration upgrade --force
```
