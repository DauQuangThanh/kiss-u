...
Technical reference for kiss extension system APIs and manifest schema.
...
requires:
  kiss_version: string  # Required, version specifier (>=X.Y.Z)
...
provides:
  commands:              # Required, at least one command
    - name: string       # Required, pattern: ^kiss\.[a-z0-9-]+\.[a-z0-9-]+$
...
hooks:                   # Optional, event hooks
  event_name:            # e.g., "after_specify", "after_plan", "after_taskify", "after_implement"
...
#### `requires.kiss_version`

- **Type**: string
- **Format**: Version specifier
- **Description**: Required kiss version range
- **Examples**:
...
- **Format**:  `kiss.{extension-id}.{command-name}`
- **Examples**: `kiss.jira.specstoissues`, `kiss.linear.sync`
- **Invalid**: `jira.specstoissues`, `kiss.command`, `kiss.jira.CreateIssues`

#### `hooks`

- **Type**: object
- **Keys**: Event names (e.g., `after_specify`, `after_plan`, `after_taskify`, `after_implement`, `before_verify-tasks`)
- **Description**: Hooks that execute at lifecycle events
- **Events**: Defined by core kiss commands
...
manifest.requires_kiss_version  # str: Required kiss version
...
CompatibilityError    # Incompatible with current kiss version
```
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "enabled": true,
      "registered_commands": ["kiss.jira.specstoissues", ...],
...
```python
from kiss_cli.extensions import CatalogEntry
...
```python
from kiss_cli.extensions import ExtensionCatalog
...
**Catalog config file** (`.kiss/extension-catalogs.yml`):
...
```python
from kiss_cli.extensions import HookExecutor
...
```python
from kiss_cli.extensions import CommandRegistrar
...
- Extension context automatically injected:
  ...
  <!-- Config: .kiss/extensions/{extension-id}/ -->
  ```
...
**File**: `.kiss/extensions/{extension-id}/{extension-id}-config.yml`
...
4. **Environment Variables** (`KISS_{EXTENSION}_*`)

### Environment Variable Pattern

Format: `KISS_{EXTENSION}_{KEY}`

Examples:

- `KISS_JIRA_PROJECT_KEY`
- `KISS_LINEAR_API_KEY`
- `KISS_GITHUB_TOKEN`
...
- `before_taskify` - Before task generation
- `after_taskify` - After task generation
- `before_implement` - Before implementation
- `after_implement` - After implementation
- `before_verify-tasks` - Before cross-artifact analysis
- `after_verify-tasks` - After cross-artifact analysis
- `before_checklist` - Before checklist generation
- `after_checklist` - After checklist generation
- `before_clarify-specs` - Before spec clarification
- `after_clarify-specs` - After spec clarification
- `before_standardize` - Before standards update
- `after_standardize` - After standards update
- `before_tasks-to-issues` - Before tasks-to-issues conversion
- `after_tasks-to-issues` - After tasks-to-issues conversion

### Hook Configuration

**In `.kiss/extensions.yml`**:
...
**Usage**: `kiss extension list [OPTIONS]`
...
**Usage**: `kiss extension catalog list`
...
**Usage**: `kiss extension catalog add URL [OPTIONS]`
...
Adds a catalog entry to `.kiss/extension-catalogs.yml`.

### extension catalog remove

**Usage**: `kiss extension catalog remove NAME`

**Arguments**:

- `NAME` - Catalog name to remove

Removes a catalog entry from `.kiss/extension-catalogs.yml`.

### extension add

**Usage**: `kiss extension add EXTENSION [OPTIONS]`
...
**Usage**: `kiss extension remove EXTENSION [OPTIONS]`
...
**Usage**: `kiss extension search [QUERY] [OPTIONS]`
...
**Usage**: `kiss extension info EXTENSION`
...
**Usage**: `kiss extension update [EXTENSION]`
...
**Usage**: `kiss extension enable EXTENSION`
...
**Usage**: `kiss extension disable EXTENSION`
...
Raised when extension is incompatible with current kiss version.

```python
from kiss_cli.extensions import CompatibilityError
...
```python
from kiss_cli.extensions import ExtensionError
...
```python
from kiss_cli.extensions import version_satisfies
...
└── scripts/                    # (existing kiss)

.claude/
└── commands/
    └── kiss.{ext}.{cmd}.md  # Registered commands
```
