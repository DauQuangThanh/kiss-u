...
Introduce an extension system to kiss that allows modular integration with external tools (Jira, Linear, Azure DevOps, etc.) without bloating the core framework. Extensions are self-contained packages installed into `.kiss/extensions/` with declarative manifests, versioned independently, and discoverable through a central catalog.
...
1. **Monolithic Growth**: Adding Jira integration to core kiss creates:
...
1. **Modularity**: Core kiss remains lean, extensions are opt-in
...
- Standard directory structure (`.kiss/extensions/{name}/`)
- Declarative manifest (`extension.yml`)
- Predictable command naming (`kiss.{extension}.{command}`)
...
- Simple installation: `kiss extension add jira`
...
│  └─────────┬────────────────────────────────────────┘   │
└────────────┼────────────────────────────────────────────┘
             │ Hook Points (after_taskify, after_implement)
             ↓
┌─────────────────────────────────────────────────────────┐
│                    Extensions                           │
│  ┌──────────────────────────────────────────────────┐   │
...
extension:
  ...
  description: "Create Jira Epics, Stories, and Issues from kiss artifacts"
  author: "Stats Perform"       # Author/organization
  repository: "https://github.com/statsperform/kiss-jira"
  license: "MIT"                # SPDX license identifier
  homepage: "https://github.com/statsperform/kiss-jira/blob/main/README.md"

# Compatibility requirements (REQUIRED)
requires:
  # Spec-kit version (semantic version range)
  kiss_version: ">=0.1.0,<2.0.0"
  ...
  # Core kiss commands this extension depends on
  commands:
    - "kiss.taskify"             # Extension needs tasks command
...
hooks:
  # Hook fired after /kiss.taskify completes
  after_taskify:
  ...
  # Hook fired after /kiss.implement completes
  after_implement:
    command: "kiss.jira.sync-status"
    optional: true
...
changelog: "https://github.com/statsperform/kiss-jira/blob/main/CHANGELOG.md"

# Support information (OPTIONAL)
support:
  documentation: "https://github.com/statsperform/kiss-jira/blob/main/docs/"
  issues: "https://github.com/statsperform/kiss-jira/issues"
  discussions: "https://github.com/statsperform/kiss-jira/discussions"
...
4. **Extract**: Unpack to `.kiss/extensions/jira/`
5. **Configure**: Copy config templates
6. **Register**: Add commands to AI agent config
7. **Record**: Update `.kiss/extensions/.registry`

**Registry Format** (`.kiss/extensions/.registry`):
...
1. AI agent finds command in `.claude/commands/kiss.jira.specstoissues.md`
...
3. Remove from `.kiss/extensions/jira/`
4. Update registry
...
**Output**: `.claude/commands/kiss.jira.specstoissues.md`
...
tools:
  ...
  - 'jira-mcp-server/story_create'
scripts:
  sh: .kiss/scripts/bash/check-prerequisites.sh --json
  ps: .kiss/scripts/powershell/check-prerequisites.ps1 -Json
---
...
## Steps
1. Load jira-config.yml from .kiss/extensions/jira/
2. Parse spec.md and tasks.md
3. Create Jira items
```
...
**Output**: `.gemini/commands/kiss.jira.specstoissues.toml`
...
**Location**: `src/kiss_cli/extensions.py`
...
def register_extension_commands(
    ...
    ext_dir = project_path / ".kiss" / "extensions" / ext_id
...
```yaml
# .kiss/extensions/jira/jira-config.yml (Project config)
project:
  key: "MSATS"
...
```yaml
# .kiss/extensions/jira/jira-config.local.yml (Local overrides - gitignored)
project:
...
```bash
config_json=$(bash .kiss/extensions/jira/scripts/parse-jira-config.sh)
echo "$config_json"
```
...
**Script**: `.kiss/extensions/jira/scripts/parse-jira-config.sh`

```bash
#!/usr/bin/env bash
set -euo pipefail

EXT_DIR=".kiss/extensions/jira"
...
if [ -n "${KISS_JIRA_PROJECT_KEY:-}" ]; then
  defaults=$(echo "$defaults" | jq ".project.key = \"$KISS_JIRA_PROJECT_KEY\"")
fi
...
schema = load_yaml(".kiss/extensions/jira/extension.yml")['config_schema']
...
**File**: `.kiss/extensions.yml` (project-level extension config)
...
**In core command** (e.g., `templates/commands/taskify.md`):
...
```bash
# Check if extensions.yml exists and has after_taskify hooks
if [ -f ".kiss/extensions.yml" ]; then
  # Parse hooks for after_taskify
  hooks=$(yq eval '.hooks.after_taskify[] | select(.enabled == true)' .kiss/extensions.yml -o=json)
...
The AI agent sees `EXECUTE_COMMAND: kiss.jira.specstoissues` in output and automatically invokes that command.
...
kiss uses two catalog files with different purposes:

#### User Catalog (`catalog.json`)

**URL**: `https://raw.githubusercontent.com/DauQuangThanh/kiss-u/main/extensions/catalog.json`
...
**URL**: `https://raw.githubusercontent.com/DauQuangThanh/kiss-u/main/extensions/catalog.community.json`
...
1. **Discover**: `kiss extension search` searches both catalogs — community extensions appear automatically
2. **Review**: Evaluate community extensions for security, quality, and organizational fit
3. **Curate**: Copy approved entries from community catalog to your `catalog.json`, or add to `.kiss/extension-catalogs.yml` with `install_allowed: true`
4. **Install**: Use `kiss extension add <name>` — only allowed from `install_allowed: true` catalogs
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "version": "1.0.0",
      "download_url": "https://github.com/statsperform/kiss-jira/releases/download/v1.0.0/kiss-jira-1.0.0.zip",
      "repository": "https://github.com/statsperform/kiss-jira",
      "homepage": "https://github.com/statsperform/kiss-jira/blob/main/README.md",
      "documentation": "https://github.com/statsperform/kiss-jira/blob/main/docs/",
      "changelog": "https://github.com/statsperform/kiss-jira/blob/main/CHANGELOG.md",
    ...
    "linear": {
      ...
      "id": "linear",
      "description": "Sync kiss tasks with Linear issues",
      "author": "Community",
      "version": "0.9.0",
      "download_url": "https://github.com/example/kiss-linear/releases/download/v0.9.0/kiss-linear-0.9.0.zip",
      "repository": "https://github.com/example/kiss-linear",
      "requires": {
        "kiss_version": ">=0.1.0"
      },
...
kiss supports a **catalog stack** — an ordered list of catalogs that the CLI merges and searches across. This allows organizations to maintain their own org-approved extensions alongside an internal catalog and community discovery, all at once.

#### Catalog Stack Resolution

The active catalog stack is resolved in this order (first match wins):

1. **`KISS_CATALOG_URL` environment variable** — single catalog replacing all defaults (backward compat)
2. **Project-level `.kiss/extension-catalogs.yml`** — full control for the project
3. **User-level `~/.kiss/extension-catalogs.yml`** — personal defaults
...
This means `kiss extension search` surfaces community extensions out of the box, while `kiss extension add` is still restricted to entries from catalogs with `install_allowed: true`.

#### `.kiss/extension-catalogs.yml` Config File
...
A user-level equivalent lives at `~/.kiss/extension-catalogs.yml`. When a project-level config is present with one or more catalog entries, it takes full control and the built-in defaults are not applied. An empty `catalogs: []` list is treated the same as no config file, falling back to defaults.
...
kiss extension catalog add --name "internal" --install-allowed \
  https://internal.company.com/kiss/catalog.json

# Add a discovery-only catalog
kiss extension catalog add --name "community" \
  https://raw.githubusercontent.com/DauQuangThanh/kiss-u/main/extensions/catalog.community.json
...
Extensions from discovery-only catalogs are shown in `kiss extension search` results but cannot be installed directly:

```text
⚠  'linear' is available in the 'community' catalog but installation is not allowed from that catalog.

To enable installation, add 'linear' to an approved catalog (install_allowed: true) in .kiss/extension-catalogs.yml.
```

#### `KISS_CATALOG_URL` (Backward Compatibility)

The `KISS_CATALOG_URL` environment variable still works — it is treated as a single `install_allowed: true` catalog, **replacing both defaults** for full backward compatibility:

```bash
# Point to your organization's catalog
export KISS_CATALOG_URL="https://internal.company.com/kiss/catalog.json"

# All extension commands now use your custom catalog
kiss extension search       # Uses custom catalog
kiss extension add jira     # Installs from custom catalog
```
...
Installed Extensions:
  ✓ Jira Integration (v1.0.0)
     jira
     Create Jira issues from kiss artifacts
     Commands: 3 | Hooks: 2 | Priority: 10 | Status: Enabled

  ✓ Linear Integration (v0.9.0)
     linear
     Create Linear issues from kiss artifacts
...
│ Jira Integration                                        │
│                                                         │
│ Create Jira Epics, Stories, and Issues from kiss   │
│ artifacts                                               │
│                                                         │
│ Author: Stats Perform                                   │
...
│ Downloads: 1,250                                        │
│                                                         │
│ Repository: github.com/statsperform/kiss-jira       │
│ Documentation: github.com/.../docs                      │
└─────────────────────────────────────────────────────────┘

Install: kiss extension add jira
```
...
Provides:
  Commands:
    ...
    • kiss.jira.discover-fields - Discover Jira custom fields
    • kiss.jira.sync-status - Sync task completion status

  Hooks:
    • after_taskify - Prompt to create Jira issues
...
✓ Downloaded kiss-jira-1.0.0.zip (245 KB)
✓ Validated manifest
✓ Checked compatibility (kiss 0.1.0 ≥ 0.1.0)
✓ Extracted to .kiss/extensions/jira/
✓ Registered 3 commands with claude
✓ Installed config template (jira-config.yml)

⚠  Configuration required:
   Edit .kiss/extensions/jira/jira-config.yml to set your Jira project key

Extension installed successfully!

Next steps:
  1. Configure: vim .kiss/extensions/jira/jira-config.yml
  2. Discover fields: /kiss.jira.discover-fields
  3. Use commands: /kiss.jira.specstoissues
```
...
⚠  This will remove:
   ...
   • Extension directory: .kiss/extensions/jira/
   • Config file: jira-config.yml (will be backed up)

Continue? (yes/no): yes

✓ Unregistered commands
✓ Backed up config to .kiss/extensions/.backup/jira-config.yml
✓ Removed extension directory
✓ Updated registry

Extension removed successfully.

To reinstall: kiss extension add jira
```
...
✓ Downloaded kiss-jira-1.1.0.zip
✓ Validated manifest
...
Changelog: https://github.com/statsperform/kiss-jira/blob/main/CHANGELOG.md#v110
```
...
#### `kiss extension enable/disable NAME`
...
#### `kiss extension set-priority NAME PRIORITY`

Change the resolution priority of an installed extension.

```bash
$ kiss extension set-priority jira 5
...
def check_compatibility(extension_manifest: dict) -> bool:
    ...
    # 1. Check kiss version
    current_speckit = get_kiss_version()  # e.g., "0.1.5"
    required_speckit = requires['kiss_version']  # e.g., ">=0.1.0,<2.0.0"

    if not version_satisfies(current_speckit, required_speckit):
        raise IncompatibleVersionError(
            f"Extension requires kiss {required_speckit}, "
    ...
    for cmd in requires.get('commands', []):
        if not command_exists(cmd):
            raise MissingCommandError(
                ...
                f"Update kiss to latest version"
            )
...
provides:
  commands:
    - name: "kiss.jira.old-command"
      file: "commands/old-command.md"
      deprecated: true
      deprecated_message: "Use kiss.jira.new-command instead"
...
```text
⚠️  Warning: /kiss.jira.old-command is deprecated
   Use /kiss.jira.new-command instead
...
- Code reviewed by kiss maintainers
...
permissions:
  - "filesystem:read:.kiss/extensions/jira/"  # Can only read own config
  - "filesystem:write:.kiss/memory/"          # Can write to memory
  - "network:external:*.atlassian.net"           # Can call Jira API
  - "env:read:KISS_JIRA_*"                    # Can read own env vars
```
...
"jira": {
  ...
  "signature": "https://github.com/.../kiss-jira-1.0.0.sig",
...
**Goal**: Existing kiss projects work without changes.

**Strategy**:

1. **Core commands unchanged**: `/kiss.taskify`, `/kiss.implement`, etc. remain in core

2. **Optional extensions**: Users opt-in to extensions

3. **Gradual migration**: Existing `tasks-to-issues` stays in core, Jira extension is alternative

4. **Deprecation timeline**:
   - **v0.2.0**: Introduce extension system, keep core `tasks-to-issues`
   - **v0.3.0**: Mark core `tasks-to-issues` as "legacy" (still works)
   - **v1.0.0**: Consider removing core `tasks-to-issues` in favor of extension

### Migration Path for Users

**Scenario 1**: User has no `tasks-to-issues` usage

- No migration needed, extensions are opt-in

**Scenario 2**: User uses core `tasks-to-issues` (GitHub Issues)
...
**Scenario 4**: User has custom scripts calling `tasks-to-issues`
...
kiss extension add github-projects
/kiss.github.taskstoissues
```
...
provides:
  commands:
    - name: "kiss.github.taskstoissues"
      file: "commands/tasks-to-issues.md"
      aliases: ["kiss.github.sync-taskstoissues"]  # Alternate namespaced entry point
```

AI agents register both names, so callers can migrate to the alternate alias without relying on deprecated global shortcuts like `/kiss.tasks-to-issues`.
...
- [x] CLI commands:
  - [x] `kiss extension list`
  - [x] `kiss extension add` (from URL and local `--dev`)
  - [x] `kiss extension remove`
- [x] Extension registry (`.kiss/extensions/.registry`)
...
- [x] Central catalog (`extensions/catalog.json` in kiss repo)
- [x] Community catalog (`extensions/catalog.community.json`)
- [x] Catalog fetch and parsing with multi-catalog support
- [x] CLI commands:
  - [x] `kiss extension search`
  - [x] `kiss extension info`
  - [x] `kiss extension catalog list`
  - [x] `kiss extension catalog add`
  - [x] `kiss extension catalog remove`
...
- [x] Project extensions config (`.kiss/extensions.yml`)
- [x] CLI commands:
  - [x] `kiss extension update` (with atomic backup/restore)
  - [x] `kiss extension enable/disable`
...
- [x] **Cache invalidation**: Cache invalidated when `KISS_CATALOG_URL` changes
...
**Decision**: **Option C** - Both prefixed and aliases are supported. Commands use `kiss.{extension}.{command}` as canonical name, with optional aliases defined in manifest.
...
**Decision**: **Option A** - Extension directory (`.kiss/extensions/{ext-id}/{ext-id}-config.yml`). This keeps extensions self-contained and easier to manage.
...
**Decision**: **Option A** - Hooks are registered in `.kiss/extensions.yml` and executed by the AI agent when it sees the hook trigger. Hook state (enabled/disabled) is managed per-extension.
...
**Complete structure of `kiss-jira` extension:**
...
| Extension Feature | kiss Version | AI Agent Support |
|-------------------|------------------|------------------|
| Basic commands | 0.2.0+ | Claude, Gemini, Copilot |
| Hooks (after_taskify) | 0.3.0+ | Claude, Gemini |
...
This RFC proposes a comprehensive extension system for kiss that:
