...
If `$ARGUMENTS` is empty, you must tell the user to provide an extension name, for example: `/kiss.selftest.extension linear`.

## Steps

### Step 1: Catalog Discovery Validation

Check if the extension exists in the kiss catalog.
...
```bash
kiss extension add "$ARGUMENTS" --from "<download_url>"
```
...
```bash
cat .kiss/extensions/.registry/$ARGUMENTS.json
```
...
test_selftest_discovery.py::test_catalog_search [PASS/FAIL]
  Details: [Provide execution result of kiss extension search]

test_selftest_installation.py::test_extension_add [PASS/FAIL]
  Details: [Provide execution result of kiss extension add]
