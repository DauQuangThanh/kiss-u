...
Extension system for [kiss](https://github.com/DauQuangThanh/kiss-u) - add new functionality without bloating the core framework.

## Extension Catalogs

kiss provides two catalog files with different purposes:

### Your Catalog (`catalog.json`)

- **Purpose**: Default upstream catalog of extensions used by the kiss CLI
- **Default State**: Empty by design in the upstream project - you or your organization populate a fork/copy with extensions you trust
- **Location (upstream)**: `extensions/catalog.json` in the GitHub-hosted kiss repo
- **CLI Default**: The `kiss extension` commands use the upstream catalog URL by default, unless overridden
- **Org Catalog**: Point `KISS_CATALOG_URL` at your organization's fork or hosted catalog JSON to use it instead of the upstream default
...
kiss extension search  # Now uses your organization's catalog instead of the upstream default
```

### Community Reference Catalog (`catalog.community.json`)

> [!NOTE]
> Community extensions are independently created and maintained by their respective authors. GitHub and the kiss maintainers may review pull requests that add entries to the community catalog for formatting, catalog structure, or policy compliance, but they do **not review, audit, endorse, or support the extension code itself**. Review extension source code before installation and use at your own discretion.
...
4. **Team members** can now discover and install them:
   - `kiss extension search` shows your curated catalog
   - `kiss extension add <name>` installs from your catalog
...
**Tradeoff**: Extensions installed this way won't appear in `kiss extension search` for other team members unless you also add them to your `catalog.json`.
...
🔍 **Browse and search community extensions on the [Community Extensions website](https://kiss-community.github.io/extensions/).**
...
kiss extension search                  # See what's in your catalog
kiss extension add <extension-name>    # Install by name

# Direct from URL (bypasses catalog)
kiss extension add <extension-name> --from https://github.com/<org>/<repo>/archive/refs/tags/<version>.zip
