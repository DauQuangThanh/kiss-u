...
This is an example command that demonstrates how to create commands for kiss extensions.
...
```bash
config_file=".kiss/extensions/my-extension/my-extension-config.yml"

if [ ! -f "$config_file" ]; then
  echo "❌ Error: Configuration not found at $config_file"
  echo "Run 'kiss extension add my-extension' to install and configure"
  exit 1
fi
...
setting_value="${KISS_MY_EXTENSION_KEY:-$setting_value}"
...
```bash
output_file=".kiss/my-extension-output.json"
...
- `KISS_MY_EXTENSION_KEY` - Overrides `settings.key`
- `KISS_MY_EXTENSION_ANOTHER_KEY` - Overrides `settings.another_key`

Example:

```bash
export KISS_MY_EXTENSION_KEY="override-value"
```
...
export KISS_MY_EXTENSION_KEY="custom-value"
> /kiss.my-extension.example
```
...
*For more information, see the extension README or run `kiss extension info my-extension`*
