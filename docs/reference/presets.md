...
Presets customize how kiss works — overriding templates, commands, and terminology without changing any tooling. They let you enforce organizational standards, adapt the workflow to your methodology, or localize the entire experience. Multiple presets can be stacked with priority ordering.

## Search Available Presets

```bash
kiss preset search [query]
```
...
> **Note:** All preset commands require a project already initialized with `kiss init`.

## Remove a Preset

```bash
kiss preset remove <preset_id>
```
...
```bash
kiss preset enable <preset_id>
kiss preset disable <preset_id>
```
...
```bash
kiss preset set-priority <preset_id> <priority>
```
...
Adds a catalog to the project's `.kiss/preset-catalogs.yml`.

### Remove a Catalog

```bash
kiss preset catalog remove <name>
```
...
1. **Environment variable** — `KISS_PRESET_CATALOG_URL` overrides all catalogs
2. **Project config** — `.kiss/preset-catalogs.yml`
3. **User config** — `~/.kiss/preset-catalogs.yml`
4. **Built-in defaults** — official catalog + community catalog

Example `.kiss/preset-catalogs.yml`:
...
flowchart TB
    subgraph stack [" "]
        direction TB
        A["⬆ Highest precedence<br/><br/>1. Project-local overrides<br/>.kiss/templates/overrides/"]
        B["2. Presets — by priority<br/>.kiss/presets/‹id›/"]
        C["3. Extensions — by priority<br/>.kiss/extensions/‹id›/"]
        D["4. kiss core<br/>.kiss/templates/<br/><br/>⬇ Lowest precedence"]
    end
...
| --------- | -------------- | ------------------------------------------ |
| Templates | `templates/`   | `.kiss/templates/overrides/`            |
| Commands  | `commands/`    | `.kiss/templates/overrides/`            |
| Scripts   | `scripts/`     | `.kiss/templates/overrides/scripts/`    |
...
```bash
kiss preset add compliance --priority 5
kiss preset add team-workflow --priority 10
```
...
Yes. Presets stack by priority — each file is resolved independently from the highest-priority source that provides it. Use `kiss preset set-priority` to control the order.

### How do I see which file is actually being used?

Run `kiss preset resolve <name>` to trace the resolution stack and see which file wins.

### What's the difference between disabling and removing a preset?

**Disabling** (`kiss preset disable`) keeps the preset installed but excludes its files from the resolution stack. Commands the preset registered remain available in your AI coding agent. This is useful for temporarily testing behavior without a preset, or comparing output with and without it. Re-enable anytime with `kiss preset enable`.

**Removing** (`kiss preset remove`) fully uninstalls the preset — deletes its files, unregisters its commands from your AI coding agent, and removes it from the registry.

### Who maintains presets?

Most presets are independently created and maintained by their respective authors. The kiss maintainers do not review, audit, endorse, or support preset code. Review a preset's source code before installing and use at your own discretion. For issues with a specific preset, contact its author or file an issue on the preset's repository.
