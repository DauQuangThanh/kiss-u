...
> **Note:** All workflow commands require a project already initialized with `kiss init`.
...
1. **Environment variable** — `KISS_WORKFLOW_CATALOG_URL` overrides all catalogs
2. **Project config** — `.kiss/workflow-catalogs.yml`
3. **User config** — `~/.kiss/workflow-catalogs.yml`
...
| Type         | Purpose                                          |
| ------------ | ------------------------------------------------ |
| `command`    | Invoke a kiss command (e.g., `kiss.plan`) |
...
| `steps.kiss.output.file`    | Output from a previous step          |
