...
_HOOK_COMMAND_NOTE = (
    ...
    "For example, `kiss.git.commit` → `/kiss-git-commit`.\n"
)
...
ARGUMENT_HINTS: dict[str, str] = {
    ...
    "taskify": "Optional task generation constraints",
    "implement": "Optional implementation guidance or task filter",
    "analyze": "Optional focus areas for analysis",
    "clarify": "Optional areas to clarify in the spec",
    "standardize": "Principles or values for project standards",
    "checklist": "Domain or focus area for the checklist",
    "tasks-to-issues": "Optional filter or label for GitHub issues",
}
...
class ClaudeIntegration(SkillsIntegration):
    ...
    def _render_skill(self, template_name: str, frontmatter: dict[str, Any], body: str) -> str:
        ...
        skill_name = f"kiss-{template_name.replace('.', '-')}"
        description = frontmatter.get(
            "description",
            f"Spec-kit workflow command: {template_name}",
        )
        skill_frontmatter = self._build_skill_fm(
            skill_name, description, f"agent-skills/{skill_name}/{skill_name}.md"
        )
    ...
    def setup(
        ...
        **opts: Any,
        ...
        for path in created:
            ...
            skill_dir_name = path.parent.name  # e.g. "kiss-plan"
            stem = skill_dir_name
            if stem.startswith("kiss-"):
                stem = stem[len("kiss-"):]
