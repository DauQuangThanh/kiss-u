...
class WorkflowRegistry:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.workflows_dir = project_root / ".kiss" / "workflows"
...
class WorkflowCatalog:
    ...
    1. ``KISS_WORKFLOW_CATALOG_URL`` env var (overrides all)
    2. Project-level ``.kiss/workflow-catalogs.yml``
    3. User-level ``~/.kiss/workflow-catalogs.yml``
    4. Built-in defaults (official + community)
    """

    # kiss is offline-only (Phase 4): only a single, bundled catalog is used.
    from .._bundled_catalogs import BUNDLED_CATALOG_URL as _BUNDLED_URL
    DEFAULT_CATALOG_URL = _BUNDLED_URL
    CACHE_DURATION = 3600  # retained for API compatibility
    ...
    def get_active_catalogs(self) -> list[WorkflowCatalogEntry]:
        ...
        kiss is offline-only (Phase 4): env-var overrides and
        ``workflow-catalogs.yml`` files are ignored.
        """
        return [
            WorkflowCatalogEntry(
                ...
                description="Built-in catalog of bundled workflows",
            ),
        ]
    ...
    def _fetch_single_catalog(
        ...
        """Return the bundled workflow catalog data.

        kiss is offline-only: this method ignores *entry.url* and
        *force_refresh* and reads only the bundled ``catalog.json``.
        """
        from .._bundled_catalogs import load_bundled_catalog
        return load_bundled_catalog("workflows")
    ...
    def add_catalog(self, url: str, name: str | None = None) -> None:
        ...
        self._validate_catalog_url(url)
        config_path = self.project_root / ".kiss" / "workflow-catalogs.yml"
