"""
Extension Manager for kiss

Handles installation, removal, and management of kiss extensions.
Extensions are modular packages that add commands and functionality to kiss
...
HOOK_PHASE_PATTERN = re.compile(r"^(before|after)_kiss-[a-z][a-z0-9-]*$")

REINSTALL_COMMAND = "uv tool install kiss --force --from git+https://github.com/DauQuangThanh/kiss-u.git"
...
class ExtensionManifest:
    ...
    def _validate(self):
        ...
        if "kiss_version" not in requires:
            raise ValidationError("Missing requires.kiss_version")
        ...
        if hooks:
            for hook_name, hook_config in hooks.items():
                ...
                # `before_kiss-<skill>` / `after_kiss-<skill>`.
                if not HOOK_PHASE_PATTERN.match(hook_name):
                    raise ValidationError(
                        f"Invalid hook phase '{hook_name}': "
                        "must match pattern '^(before|after)_kiss-[a-z][a-z0-9-]*$'"
                    )
        ...
        for cmd in commands:
            ...
            if not EXTENSION_COMMAND_NAME_PATTERN.match(cmd["name"]):
                raise ValidationError(
                    ...
                    "must follow pattern 'kiss.{extension}.{command}'"
                )
            ...
            # (e.g. 'kiss.verify' short aliases used by existing extensions).
        ...
        # an alias-form ref (ext.cmd → kiss.ext.cmd).  Always emit a warning when
        ...
        for hook_name, hook_data in self.data.get("hooks", {}).items():
            ...
            # Step 2: lift alias-form '{ext_id}.cmd' to canonical 'kiss.{ext_id}.cmd'.
            parts = after_rename.split(".")
            if len(parts) == 2 and parts[0] == ext["id"]:
                final_ref = f"kiss.{ext['id']}.{parts[1]}"
            else:
...
class ExtensionRegistry:
    ...
    def __init__(self, extensions_dir: Path):
        ...
        Args:
            extensions_dir: Path to .kiss/extensions/ directory
        """
...
class ExtensionManager:
    ...
    def _collect_manifest_command_names(manifest: ExtensionManifest) -> Dict[str, str]:
        ...
        - primary commands must use the canonical `kiss.{extension}.{command}` shape
    ...
    def _register_extension_skills(
        ...
        for cmd_info in manifest.commands:
            ...
            if short_name_raw.startswith("kiss."):
                short_name_raw = short_name_raw[len("kiss."):]
            skill_name = f"kiss-{short_name_raw.replace('.', '-')}"
    ...
    def check_compatibility(
        ...
        """Check if extension is compatible with current kiss version.
        ...
        try:
            ...
            if current not in specifier:
                raise CompatibilityError(
                    f"Extension requires kiss {required}, "
    ...
    def install_from_directory(
        ...
        if self.registry.is_installed(manifest.id):
            raise ExtensionError(
                ...
                f"Use 'kiss extension remove {manifest.id}' first."
            )
    ...
    def install_from_zip(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(extension_dir, kiss_version, priority=priority)
...
class CommandRegistrar:
    ...
    def register_commands_for_agent(
        ...
        context_note = f"\n<!-- Extension: {manifest.id} -->\n<!-- Config: .kiss/extensions/{manifest.id}/ -->\n"
...
class ExtensionCatalog:
    ...
    # kiss is offline-only (Phase 4): the only catalog is bundled.  A sentinel
    # URL is kept so code that still inspects ``entry.url`` has a non-empty
    # string; it is never dereferenced.
    ...
    def get_active_catalogs(self) -> List[CatalogEntry]:
        ...
        ``extension-catalogs.yml`` files are ignored.
        """
        return [
            CatalogEntry(
                ...
                description="Built-in catalog of bundled extensions",
            ),
        ]
    ...
    def _fetch_single_catalog(self, entry: CatalogEntry, force_refresh: bool = False) -> Dict[str, Any]:
        """Return the bundled extension catalog data.
        ...
        *force_refresh* and reads only the ``catalog.json`` that ships with
        ...
        return load_bundled_catalog("extensions")
    ...
    def download_extension(self, extension_id: str, target_dir: Optional[Path] = None) -> Path:
        ...
        Retained for API compatibility.  Bundled extensions are installed
        directly from ``_locate_bundled_extension()`` in the CLI layer.
        """
        ...
        if ext_info.get("bundled"):
            raise ExtensionError(
                f"Extension '{extension_id}' is bundled with kiss. "
                f"It should be installed from the local package. "
                f"Try reinstalling: {REINSTALL_COMMAND}"
            )

        raise ExtensionError(
            f"Extension '{extension_id}' cannot be downloaded: kiss is offline-only."
        )

    def clear_cache(self):
        """Clear the bundled catalog cache files."""
...
class ConfigManager:
    ...
    2. Project config (.kiss/extensions/{ext-id}/{ext-id}-config.yml)
    3. Local config (.kiss/extensions/{ext-id}/local-config.yml) - gitignored
    4. Environment variables (KISS_{EXT_ID}_{KEY})
    """
    ...
    def __init__(self, project_root: Path, extension_id: str):
        ...
        self.extension_dir = project_root / ".kiss" / "extensions" / extension_id
    ...
    def _get_env_config(self) -> Dict[str, Any]:
        ...
        KISS_{EXT_ID}_{SECTION}_{KEY}

        For example:
        - KISS_JIRA_CONNECTION_URL
...
class HookExecutor:
    ...
    def __init__(self, project_root: Path):
        ...
        self.project_root = project_root
        self.extensions_dir = project_root / ".kiss" / "extensions"
        self.config_file = project_root / ".kiss" / "extensions.yml"
    ...
    def _load_init_options(self) -> Dict[str, Any]:
        ...
        Uses the shared helper from kiss_cli and caches values per executor
    ...
    def _skill_name_from_command(command: Any) -> str:
        """Map a command id like kiss.plan to kiss-plan skill name."""
        if not isinstance(command, str):
            return ""
        command_id = command.strip()
        if not command_id.startswith("kiss."):
            return ""
        return f"kiss-{command_id[len('kiss.'):].replace('.', '-')}"
