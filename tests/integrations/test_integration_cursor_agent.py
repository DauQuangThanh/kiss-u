...
class TestCursorAgentIntegrationInstall:
    """--integration cursor-agent installs the cursor-agent integration."""

    def test_integration_cursor_agent_installs_successfully(self, tmp_path):
        """--integration cursor-agent should install the cursor-agent integration and exit 0."""
        from typer.testing import CliRunner
        from kiss_cli import app

        runner = CliRunner()
        target = tmp_path / "test-proj"
        result = runner.invoke(app, ["init", str(target), "--integration", "cursor-agent", "--no-git", "--ignore-agent-tools", ])

        assert result.exit_code == 0, f"init --integration cursor-agent failed: {result.output}"
        assert (target / ".cursor" / "skills" / "kiss-plan" / "SKILL.md").exists()
