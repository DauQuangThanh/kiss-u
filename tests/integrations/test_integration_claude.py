...
from kiss_cli.integrations.claude import ARGUMENT_HINTS
...
class TestClaudeIntegration:
    ...
    def test_config_uses_skills(self):
        ...
        assert integration.config["skills_subdir"] == "skills"
    ...
    def test_setup_creates_skill_files(self, tmp_path):
        ...
        plan_skill = skills_dir / "kiss-plan" / "SKILL.md"
        ...
        assert parsed["name"] == "kiss-plan"
        assert parsed["user-invocable"] is True
        assert parsed["disable-model-invocation"] is False
        assert parsed["metadata"]["source"] == "agent-skills/kiss-plan/kiss-plan.md"
    ...
    def test_remove_context_section_strips_bom(self, tmp_path):
        ...
        assert b"<!-- KISS" not in remaining
    ...
    def test_ai_flag_auto_promotes_and_enables_skills(self, tmp_path):
        ...
        init_options = json.loads(
            (project / ".kiss" / "init-options.json").read_text(encoding="utf-8")
        )
    ...
    def test_integration_flag_creates_skill_files(self, tmp_path):
        ...
        assert result.exit_code == 0, result.output
        assert (project / ".claude" / "skills" / "kiss-specify" / "SKILL.md").exists()
    ...
    def test_interactive_claude_selection_uses_integration_path(self, tmp_path):
        ...
        skill_file = project / ".claude" / "skills" / "kiss-plan" / "SKILL.md"
    ...
    def test_claude_init_remains_usable_when_converter_fails(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["init", str(target), "--integration", "claude", "--no-git", "--ignore-agent-tools"],
        )

        assert result.exit_code == 0
        assert (target / ".claude" / "skills" / "kiss-specify" / "SKILL.md").exists()
    ...
    def test_claude_hooks_render_skill_invocation(self, tmp_path):
        ...
        project.mkdir()
        init_options = project / ".kiss" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        init_options.write_text(json.dumps({"integration": "claude", "ai_skills": True}))
        ...
        assert "Executing: `/kiss-plan`" in message
        assert "EXECUTE_COMMAND: kiss.plan" in message
        assert "EXECUTE_COMMAND_INVOCATION: /kiss-plan" in message
    ...
    def test_claude_preset_creates_new_skill_without_commands_dir(self, tmp_path):
        ...
        project.mkdir()
        save_init_options(project, {"integration": "claude", "ai_skills": True, "script": "sh"})
        ...
        (preset_dir / "commands" / "kiss.research.md").write_text(
            "---\n"
                ...
                "templates": [
                    {
                        "type": "command",
                        "name": "kiss.research",
                        "file": "commands/kiss.research.md",
                    }
                ]
            },
        }
        ...
        skill_file = skills_dir / "kiss-research" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text(encoding="utf-8")
        assert "preset:claude-skill-command" in content
        assert "name: kiss-research" in content
        assert "user-invocable: true" in content
        assert "disable-model-invocation: false" in content

        metadata = manager.registry.get("claude-skill-command")
        assert "kiss-research" in metadata.get("registered_skills", [])


class TestClaudeArgumentHints:
    """Verify that argument-hint frontmatter is injected for Claude skills."""

    def _hinted_skill_files(self, tmp_path):
        """Run setup and return only the SKILL.md files for skills that have
        a defined argument-hint in ``ARGUMENT_HINTS``. Other skills (e.g.
        the role-skill bundles) intentionally have no hint."""
        ...
        for f in skill_files:
            stem = f.parent.name.removeprefix("kiss-")
            if stem in ARGUMENT_HINTS:
                hinted.append(f)
        return hinted

    def test_all_skills_have_hints(self, tmp_path):
        """Every skill in ARGUMENT_HINTS must have an argument-hint line."""
        skill_files = self._hinted_skill_files(tmp_path)
    ...
    def test_hints_match_expected_values(self, tmp_path):
        """Each hinted skill's argument-hint must match the expected text."""
        skill_files = self._hinted_skill_files(tmp_path)
        for f in skill_files:
            stem = f.parent.name.removeprefix("kiss-")
...
class TestClaudeDisableModelInvocation:
    ...
    def test_non_claude_agents_lack_disable_model_invocation(self, tmp_path):
        ...
        fm = CommandRegistrar.build_skill_frontmatter(
            "codex", "kiss-plan", "desc", "templates/commands/plan.md"
        )
...
class TestClaudeHookCommandNote:
    ...
    def test_hook_note_injected_in_skills_with_hooks(self, tmp_path):
        ...
        m = IntegrationManifest("claude", tmp_path)
        created = i.setup(tmp_path, m, script_type="sh")
        specify_skill = tmp_path / ".claude/skills/kiss-specify/SKILL.md"
        assert specify_skill.exists()
        content = specify_skill.read_text(encoding="utf-8")
        # specify.md has hook sections
        assert "replace dots" in content, (
            "kiss-specify should have dot-to-hyphen hook note"
        )
