...
from kiss_cli.integrations.base import TomlIntegration
...
class TomlIntegrationTests:
    ...
    def _command_files(created, commands_dest):
        """Filter ``created`` paths down to the installed command ``.toml`` files.

        Each TOML command lives at
        ``<commands_dest>/kiss.<stem>/kiss.<stem>.toml``. Bundled scripts
        and templates are siblings under the same per-skill folder and
        must be excluded.
        """
    ...
    def test_setup_writes_to_correct_directory(self, tmp_path):
        ...
        for f in cmd_files:
            # Per-skill folder layout: command sits one level deep.
    ...
    def test_integration_flag_creates_files(self, tmp_path):
        ...
        commands = sorted(cmd_dir.rglob("kiss.*.toml"))
    ...
    def _expected_files(self) -> list[str]:
        ...
        ext = i.registrar_config.get("extension", ".toml")
