...
from kiss_cli.integrations.base import (
    IntegrationBase,
    IntegrationOption,
    MarkdownIntegration,
)
...
class TestIntegrationBase:
    ...
    def test_setup_uses_shared_templates(self, tmp_path):
        ...
        command_files = [f for f in created if f.name.startswith("kiss.") and f.suffix == ".md"]
        assert len(command_files) > 0
        commands_root = tmp_path / ".stub" / "commands"
        for f in command_files:
            # Each command lives in its own per-skill folder named after the command file.
            assert f.parent.parent == commands_root
            assert f.parent.name == f.stem
    ...
    def test_setup_copies_templates(self, tmp_path, monkeypatch):
        ...
        project.mkdir()
        i.setup(project, IntegrationManifest("stub", project))
        # Per-skill folder layout: .stub/commands/kiss.<stem>/kiss.<stem>.md
        assert (project / ".stub" / "commands" / "kiss.plan" / "kiss.plan.md").exists()
        assert (project / ".stub" / "commands" / "kiss.specify" / "kiss.specify.md").exists()
...
class TestBasePrimitives:
    ...
    def test_command_filename_default(self):
        i = StubIntegration()
        assert i.command_filename("plan") == "kiss.plan.md"
    ...
    def test_copy_command_to_directory(self, tmp_path):
        ...
        result = IntegrationBase.copy_command_to_directory(src, dest_dir, "kiss.plan.md")
        assert result == dest_dir / "kiss.plan.md"
    ...
    def test_setup_copies_shared_templates(self, tmp_path):
        ...
        assert len(command_files) > 0
        for f in command_files:
            # kiss.<stem>.md nested inside its per-skill folder named kiss.<stem>
            assert f.parent.name == f.stem
            assert f.parent.parent.name == "commands"


class TestBuildContextSection:
    """Pin the content the agent context file (CLAUDE.md / AGENTS.md / etc.)
    receives between the KISS markers. The section is intentionally terse —
    it tells the agent to read ``.kiss/context.yml`` (without echoing it)
    and points to the current plan for everything else."""

    def test_directs_agent_to_read_context_yml(self):
        section = IntegrationBase._build_context_section()
        assert "`.kiss/context.yml`" in section
        assert "Read `.kiss/context.yml`" in section

    def test_does_not_print_context_yml_directive(self):
        """The section must instruct the agent NOT to dump context.yml
        contents — keeps the chat output clean."""
        section = IntegrationBase._build_context_section()
        assert "do not print out the contents of `.kiss/context.yml`" in section

    def test_preserves_legacy_plan_pointer_phrase(self):
        """The phrase 'read the current plan' is asserted by integration
        tests; it must keep landing in the section verbatim."""
        section = IntegrationBase._build_context_section()
        assert "read the current plan" in section

    def test_plan_path_appended_when_supplied(self):
        section = IntegrationBase._build_context_section("specs/foo/plan.md")
        assert "at specs/foo/plan.md." in section

    def test_plan_path_omitted_when_blank(self):
        section = IntegrationBase._build_context_section()
        # No "at <path>" line should be present when plan_path is empty.
        assert "\nat " not in section
