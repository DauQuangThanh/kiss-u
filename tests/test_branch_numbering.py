...
class TestSaveBranchNumbering:
    ...
    def test_save_branch_numbering_timestamp(self, tmp_path: Path):
        opts = {"branch_numbering": "timestamp", "integration": "claude"}
        save_init_options(tmp_path, opts)

        saved = json.loads((tmp_path / ".kiss/init-options.json").read_text())
        assert saved["branch_numbering"] == "timestamp"

    def test_save_branch_numbering_sequential(self, tmp_path: Path):
        opts = {"branch_numbering": "sequential", "integration": "claude"}
        save_init_options(tmp_path, opts)
    ...
    def test_branch_numbering_defaults_to_sequential(self, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(app, ["init", str(project_dir), "--integration", "claude", "--ignore-agent-tools", "--no-git", ])
        assert result.exit_code == 0

        saved = json.loads((project_dir / ".kiss/init-options.json").read_text())
        assert saved["branch_numbering"] == "sequential"
...
class TestBranchNumberingValidation:
    ...
    def test_valid_branch_numbering_sequential(self, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(app, ["init", str(tmp_path / "proj"), "--integration", "claude", "--branch-numbering", "sequential", "--ignore-agent-tools", "--no-git", ])
        assert result.exit_code == 0
    ...
    def test_valid_branch_numbering_timestamp(self, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(app, ["init", str(tmp_path / "proj"), "--integration", "claude", "--branch-numbering", "timestamp", "--ignore-agent-tools", "--no-git", ])
        assert result.exit_code == 0
        assert "Invalid --branch-numbering" not in (result.output or "")
