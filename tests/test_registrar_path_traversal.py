...
class TestPrimaryCommandTraversal:
    ...
    @pytest.mark.parametrize("bad_name", TRAVERSAL_PAYLOADS)
    def test_copilot_safe_with_traversal_in_primary_name(self, tmp_path, bad_name):
        """copilot uses the SKILL.md layout — ``..`` segments in
        command names are neutralised (dots → hyphens) so registration
        either raises or completes without escaping the skills directory.
        """
        project, ext_dir = _project_and_source(tmp_path)
...
class TestAliasTraversal:
    ...
    @pytest.mark.parametrize("bad_alias", TRAVERSAL_PAYLOADS)
    def test_copilot_safe_with_traversal_in_alias(self, tmp_path, bad_alias):
        """Aliases on copilot go through the same SKILL.md
        sanitisation as primary names — registration must complete
        without leaking files outside the project tree.
        """
        project, ext_dir = _project_and_source(tmp_path)
        (project / ".github" / "skills").mkdir(parents=True)
...
class TestSafeRegistration:
    ...
    def test_safe_command_and_alias_still_register(self, tmp_path):
        ...
        registered = registrar.register_commands(
            "claude",
            [_cmd("kiss.myext.hello", ["kiss.myext.hi"])],
            "myext",
            ext_dir,
            project,
        )

        assert "kiss.myext.hello" in registered
        assert "kiss.myext.hi" in registered
        assert (
            project
            / ".claude"
            / "skills"
