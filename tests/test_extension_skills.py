...
Tests cover:
- SKILL.md generation when ai_skills is true in init-options.json (for SkillsIntegrations)
- No skills created when ai_skills is false or not set
...
def _create_init_options(project_root: Path, ai: str = "claude", ai_skills: bool = True):
    """Write a .kiss/init-options.json file."""
    opts_dir = project_root / ".kiss"
...
def _create_skills_dir(project_root: Path, ai: str = "claude") -> Path:
    ...
    # Match the logic in _get_skills_dir() from kiss_cli
...
def _create_extension_dir(temp_dir: Path, ext_id: str = "test-ext") -> Path:
    ...
    manifest_data = {
        ...
        "provides": {
            "commands": [
                {
                    "name": f"kiss.{ext_id}.hello",
                    "file": "commands/hello.md",
                    "description": "Test hello command",
                },
                {
                    "name": f"kiss.{ext_id}.world",
...
def skills_project(project_dir):
    """Create a project with ai_skills enabled and skills directory."""
...
def no_skills_project(project_dir):
    """Create a project without ai_skills."""
...
class TestExtensionSkillRegistration:
    ...
    def test_skills_created_when_ai_skills_active(self, skills_project, extension_dir):
        ...
        assert "kiss-test-ext-hello" in skill_dirs
        assert "kiss-test-ext-world" in skill_dirs
    ...
    def test_skill_md_content_correct(self, skills_project, extension_dir):
        ...
        assert "author: github-kiss" in content
    ...
    def test_skill_md_has_parseable_yaml(self, skills_project, extension_dir):
        ...
        assert parsed["name"] == "kiss-test-ext-hello"
    ...
    def test_existing_skill_not_overwritten(self, skills_project, extension_dir):
        ...
        custom_dir = skills_dir / "kiss-test-ext-hello"
        ...
        metadata = manager.registry.get(manifest.id)
        assert "kiss-test-ext-world" in metadata["registered_skills"]
        # The pre-existing one should NOT be in registered_skills (it was skipped)
        assert "kiss-test-ext-hello" not in metadata["registered_skills"]
    ...
    def test_registered_skills_in_registry(self, skills_project, extension_dir):
        ...
        assert "kiss-test-ext-hello" in metadata["registered_skills"]
        assert "kiss-test-ext-world" in metadata["registered_skills"]
    ...
    def test_skill_registration_resolves_script_placeholders(self, project_dir, temp_dir):
        ...
        (ext_dir / "commands" / "plan.md").write_text(
            ...
            "---\n\n"
            "Run {SCRIPT}\n"
            "Review templates/checklist.md and memory/standardize.md for __AGENT__.\n"
        )

        manager = ExtensionManager(project_dir)
        manager.install_from_directory(ext_dir, "0.1.0", register_commands=False)

        content = (skills_dir / "kiss-scripted-ext-plan" / "SKILL.md").read_text()
        assert "{SCRIPT}" not in content
        assert "{ARGS}" not in content
        assert "__AGENT__" not in content
        # Scripts/templates ship inside each skill folder now, so
        # ``../../scripts/...`` is flattened to skill-relative paths.
        assert 'scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
        assert "templates/checklist.md" in content
        assert "memory/standardize.md" in content
    ...
    def test_missing_command_file_skipped(self, skills_project, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "kiss.missing-cmd-ext.exists",
                        "file": "commands/exists.md",
                        "description": "Exists",
                    },
                    {
                        "name": "kiss.missing-cmd-ext.ghost",
        ...
        metadata = manager.registry.get(manifest.id)
        assert "kiss-missing-cmd-ext-exists" in metadata["registered_skills"]
        assert "kiss-missing-cmd-ext-ghost" not in metadata["registered_skills"]
...
class TestExtensionSkillUnregistration:
    ...
    def test_skills_removed_on_extension_remove(self, skills_project, extension_dir):
        ...
        # Verify skills exist
        assert (skills_dir / "kiss-test-ext-hello" / "SKILL.md").exists()
        assert (skills_dir / "kiss-test-ext-world" / "SKILL.md").exists()
        ...
        assert not (skills_dir / "kiss-test-ext-hello").exists()
        assert not (skills_dir / "kiss-test-ext-world").exists()
    ...
    def test_remove_handles_already_deleted_skills(self, skills_project, extension_dir):
        ...
        shutil.rmtree(skills_dir / "kiss-test-ext-hello")
        shutil.rmtree(skills_dir / "kiss-test-ext-world")
...
class TestExtensionSkillEdgeCases:
    ...
    def test_command_without_frontmatter(self, skills_project, temp_dir):
        ...
        skill_file = skills_dir / "kiss-nofm-ext-plain" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        assert "name: kiss-nofm-ext-plain" in content
        # Fallback description when no frontmatter description
        assert "Extension command: kiss.nofm-ext.plain" in content
    ...
    def test_multiple_extensions_independent_skills(self, skills_project, temp_dir):
        ...
        assert (skills_dir / "kiss-ext-a-hello" / "SKILL.md").exists()
        assert (skills_dir / "kiss-ext-b-hello" / "SKILL.md").exists()

        # Remove ext-a
        manager.remove("ext-a", keep_config=False)

        # ext-a skills gone, ext-b skills preserved
        assert not (skills_dir / "kiss-ext-a-hello").exists()
        assert (skills_dir / "kiss-ext-b-hello" / "SKILL.md").exists()
    ...
    def test_malformed_frontmatter_handled(self, skills_project, temp_dir):
        ...
        skill_file = skills_dir / "kiss-badfm-ext-broken" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        # Fallback description since frontmatter was invalid
        assert "Extension command: kiss.badfm-ext.broken" in content
    ...
    def test_remove_cleans_up_when_init_options_deleted(self, skills_project, extension_dir):
        ...
        init_opts = project_dir / ".kiss" / "init-options.json"
        init_opts.unlink()
