...
This directory contains a mock project to verify that LLM agents correctly identify and execute hook commands defined in `.kiss/extensions.yml`.
...
2. Ask it to generate tasks for the current directory:
   > "Please follow `/kiss.tasks` for the `./tests/hooks` directory."
3. **Expected Behavior**:
   - Before doing any generation, the LLM should notice the `AUTOMATIC Pre-Hook` in `.kiss/extensions.yml` under `before_tasks`.
...
1. In the same (or new) chat, ask the LLM to implement the tasks:
   > "Please follow `/kiss.implement` for the `./tests/hooks` directory."
2. **Expected Behavior**:
