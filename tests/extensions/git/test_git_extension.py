...
CORE_COMMON_SH = PROJECT_ROOT / "agent-skills" / "_template" / "scripts" / "bash" / "common.sh"
CORE_COMMON_PS = PROJECT_ROOT / "agent-skills" / "_template" / "scripts" / "powershell" / "common.ps1"
...
def _setup_project(tmp_path: Path, *, git: bool = True) -> Path:
    """Create a project directory with core scripts and .kiss."""
    ...
    ext_bash = tmp_path / ".kiss" / "extensions" / "git" / "scripts" / "bash"
    ...
    # Copy extension.yml
    shutil.copy(EXT_DIR / "extension.yml", tmp_path / ".kiss" / "extensions" / "git" / "extension.yml")
...
def _write_config(project: Path, content: str) -> Path:
    ...
    config_path = project / ".kiss" / "extensions" / "git" / "git-config.yml"
...
def _run_bash(script_name: str, cwd: Path, *args: str, env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    script = cwd / ".kiss" / "extensions" / "git" / "scripts" / "bash" / script_name
...
def _run_pwsh(script_name: str, cwd: Path, *args: str) -> subprocess.CompletedProcess:
    ...
    script = cwd / ".kiss" / "extensions" / "git" / "scripts" / "powershell" / script_name
...
class TestGitExtensionManifest:
    ...
    def test_manifest_commands(self):
        ...
        assert "kiss.git.feature" in names
        assert "kiss.git.validate" in names
        assert "kiss.git.remote" in names
        assert "kiss.git.initialize" in names
        assert "kiss.git.commit" in names
    ...
    def test_manifest_hooks(self):
        ...
        m = ExtensionManifest(EXT_DIR / "extension.yml")
        assert "before_kiss-standardize" in m.hooks
        assert "before_kiss-specify" in m.hooks
        assert "after_kiss-specify" in m.hooks
        assert "after_kiss-implement" in m.hooks
        assert m.hooks["before_kiss-standardize"]["command"] == "kiss.git.initialize"
        assert m.hooks["before_kiss-specify"]["command"] == "kiss.git.feature"
...
class TestGitExtensionInstall:
    ...
    def test_install_copies_scripts(self, tmp_path: Path):
        ...
        ext_installed = tmp_path / ".kiss" / "extensions" / "git"
    ...
    def test_bundled_extension_locator(self):
        ...
        from kiss_cli.installer import _locate_bundled_extension
...
class TestAutoCommitBash:
    def test_disabled_by_default(self, tmp_path: Path):
        ...
        result = _run_bash("auto-commit.sh", project, "after_kiss-specify")
        assert result.returncode == 0
    ...
    def test_custom_message(self, tmp_path: Path):
        ...
        result = _run_bash("auto-commit.sh", project, "after_kiss-plan")
        assert result.returncode == 0
    ...
    def test_default_true_with_no_event_key(self, tmp_path: Path):
        ...
        result = _run_bash("auto-commit.sh", project, "after_kiss-taskify")
        assert result.returncode == 0
        ...
        assert "Auto-commit after kiss-taskify" in log.stdout
    ...
    def test_no_config_file_skips(self, tmp_path: Path):
        ...
        config = project / ".kiss" / "extensions" / "git" / "git-config.yml"
...
class TestAutoCommitPowerShell:
    def test_disabled_by_default(self, tmp_path: Path):
        ...
        project = _setup_project(tmp_path)
        _write_config(project, "auto_commit:\n  default: false\n")
        result = _run_pwsh("auto-commit.ps1", project, "after_kiss-specify")
        assert result.returncode == 0
    ...
    def test_success_message_no_unicode_checkmark(self, tmp_path: Path):
        ...
        (project / "new-file.txt").write_text("content")
        result = _run_pwsh("auto-commit.ps1", project, "after_kiss-plan")
        assert result.returncode == 0
...
class TestGitCommonBash:
    def test_has_git_true(self, tmp_path: Path):
        ...
        project = _setup_project(tmp_path, git=True)
        script = project / ".kiss" / "extensions" / "git" / "scripts" / "bash" / "git-common.sh"
...
class TestGitCommonPowerShell:
    def test_test_feature_branch_accepts_single_prefix(self, tmp_path: Path):
        project = _setup_project(tmp_path)
        script = project / ".kiss" / "extensions" / "git" / "scripts" / "powershell" / "git-common.ps1"
        result = subprocess.run(
            [
                "pwsh",
                "-NoProfile",
