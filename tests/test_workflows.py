...
def project_dir(temp_dir):
    """Create a mock kiss project with .kiss/ directory."""
    kiss_dir = temp_dir / ".kiss"
    kiss_dir.mkdir()
    (kiss_dir / "workflows").mkdir()
    return temp_dir
...
def sample_workflow_file(project_dir, sample_workflow_yaml):
    ...
    wf_dir = project_dir / ".kiss" / "workflows" / "test-workflow"
...
class TestStepRegistry:
    ...
    def test_registry_populated(self):
        from kiss_cli.workflows import STEP_REGISTRY
    ...
    def test_get_step_type(self):
        from kiss_cli.workflows import get_step_type
    ...
    def test_register_step_duplicate_raises(self):
        from kiss_cli.workflows import _register_step
    ...
    def test_register_step_empty_key_raises(self):
        from kiss_cli.workflows import _register_step
        from kiss_cli.workflows.base import StepBase, StepResult
...
class TestBaseClasses:
    ...
    def test_step_context_defaults(self):
        from kiss_cli.workflows.base import StepContext
    ...
    def test_step_result_defaults(self):
        from kiss_cli.workflows.base import StepResult, StepStatus
    ...
    def test_step_status_values(self):
        from kiss_cli.workflows.base import StepStatus
    ...
    def test_run_status_values(self):
        from kiss_cli.workflows.base import RunStatus
...
class TestExpressions:
    ...
    def test_condition_evaluation(self):
        from kiss_cli.workflows.expressions import evaluate_condition
        from kiss_cli.workflows.base import StepContext
...
class TestCommandStep:
    ...
    def test_execute_basic(self):
        ...
        with patch("kiss_cli.workflows.steps.command.shutil.which", return_value=None):
            result = step.execute(config, ctx)
        assert result.status == StepStatus.FAILED
        assert result.output["command"] == "kiss.specify"
    ...
    def test_dispatch_with_mock_cli(self, tmp_path, monkeypatch):
        ...
        # Claude is a SkillsIntegration so uses /kiss-specify
        assert "/kiss-specify login" in call_args[0][0][2]
...
class TestPromptStep:
    ...
    def test_execute_basic(self):
        from unittest.mock import patch
        from kiss_cli.workflows.steps.prompt import PromptStep
        from kiss_cli.workflows.base import StepContext, StepStatus
        ...
        with patch("kiss_cli.workflows.steps.prompt.shutil.which", return_value=None):
            result = step.execute(config, ctx)
        assert result.status == StepStatus.FAILED
    ...
    def test_dispatch_with_mock_cli(self, tmp_path):
        ...
        with patch("kiss_cli.workflows.steps.prompt.shutil.which", return_value="/usr/local/bin/claude"), \
...
class TestGateStep:
    ...
    def test_execute_returns_paused(self):
        from kiss_cli.workflows.steps.gate import GateStep
        from kiss_cli.workflows.base import StepContext, StepStatus
...
class TestIfThenStep:
    ...
    def test_execute_then_branch(self):
        from kiss_cli.workflows.steps.if_then import IfThenStep
        from kiss_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "then": [{"id": "a", "command": "kiss.taskify"}],
            "else": [{"id": "b", "command": "kiss.plan"}],
        }
        result = step.execute(config, ctx)
...
class TestSwitchStep:
    ...
    def test_execute_matches_case(self):
        from kiss_cli.workflows.steps.switch import SwitchStep
        from kiss_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "cases": {
                "approve": [{"id": "plan", "command": "kiss.plan"}],
...
class TestWhileStep:
    ...
    def test_execute_condition_true(self):
        from kiss_cli.workflows.steps.while_loop import WhileStep
        from kiss_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "max_iterations": 5,
            "steps": [{"id": "fix", "command": "kiss.implement"}],
        }
        result = step.execute(config, ctx)
        assert result.output["condition_result"] is True
        assert len(result.next_steps) == 1
...
class TestDoWhileStep:
    ...
    def test_execute_always_runs_once(self):
        from kiss_cli.workflows.steps.do_while import DoWhileStep
        from kiss_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "condition": "{{ false }}",
            "max_iterations": 3,
            "steps": [{"id": "refine", "command": "kiss.specify"}],
        }
        result = step.execute(config, ctx)
        assert len(result.next_steps) == 1
    ...
    def test_execute_with_true_condition(self):
        ...
        config = {
            ...
            "condition": "{{ true }}",
            "max_iterations": 5,
            "steps": [{"id": "work", "command": "kiss.plan"}],
        }
        result = step.execute(config, ctx)
...
class TestFanOutStep:
    ...
    def test_execute_with_items(self):
        from kiss_cli.workflows.steps.fan_out import FanOutStep
        from kiss_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "max_concurrency": 3,
            "step": {"id": "impl", "command": "kiss.implement"},
        }
        result = step.execute(config, ctx)
...
class TestFanInStep:
    ...
    def test_execute_collects_results(self):
        from kiss_cli.workflows.steps.fan_in import FanInStep
        from kiss_cli.workflows.base import StepContext
...
class TestWorkflowDefinition:
    ...
    def test_from_yaml(self, sample_workflow_file):
        from kiss_cli.workflows.engine import WorkflowDefinition
...
class TestWorkflowValidation:
    ...
    def test_valid_workflow(self, sample_workflow_yaml):
        from kiss_cli.workflows.engine import WorkflowDefinition, validate_workflow
...
class TestWorkflowEngine:
    ...
    def test_load_from_file(self, sample_workflow_file, project_dir):
        from kiss_cli.workflows.engine import WorkflowEngine
    ...
    def test_execute_simple_workflow(self, project_dir):
        from unittest.mock import patch
        from kiss_cli.workflows.engine import WorkflowEngine, WorkflowDefinition
        from kiss_cli.workflows.base import RunStatus
...
"""
        ...
        assert state.step_results["step-one"]["output"]["command"] == "kiss.specify"
...
class TestRunState:
    ...
    def test_save_and_load(self, project_dir):
        from kiss_cli.workflows.engine import RunState
        from kiss_cli.workflows.base import RunStatus
...
class TestWorkflowRegistry:
    ...
    def test_add_and_get(self, project_dir):
        from kiss_cli.workflows.catalog import WorkflowRegistry
...
steps:
  - id: specify
    type: shell
    run: "echo kiss.specify {{ inputs.feature }}"
