...
# Core scripts live in each skill's bundle under agent-skills/.
# create-new-feature.{sh,ps1} ships with kiss-specify.
CREATE_FEATURE = (
    PROJECT_ROOT / "agent-skills" / "kiss-specify" / "scripts" / "bash" / "create-new-feature.sh"
)
CREATE_FEATURE_PS = (
    PROJECT_ROOT / "agent-skills" / "kiss-specify" / "scripts" / "powershell" / "create-new-feature.ps1"
)
# common.{sh,ps1} is duplicated into every skill that uses it; pick one copy.
COMMON_SH = (
    PROJECT_ROOT / "agent-skills" / "kiss-specify" / "scripts" / "bash" / "common.sh"
)
COMMON_PS = (
    PROJECT_ROOT / "agent-skills" / "kiss-specify" / "scripts" / "powershell" / "common.ps1"
)
EXT_CREATE_FEATURE = (
...
def git_repo(tmp_path: Path) -> Path:
    """Create a temp git repo with scripts and .kiss dir."""
    ...
    (tmp_path / ".kiss" / "templates").mkdir(parents=True)
    return tmp_path
...
def ext_git_repo(tmp_path: Path) -> Path:
    ...
    # Extension script needs common.sh at .kiss/scripts/bash/
    specify_scripts = tmp_path / ".kiss" / "scripts" / "bash"
    ...
    ext_dir = tmp_path / ".kiss" / "extensions" / "git" / "scripts" / "bash"
    ...
    (tmp_path / ".kiss" / "templates").mkdir(parents=True, exist_ok=True)
...
def ext_ps_git_repo(tmp_path: Path) -> Path:
    ...
    # Also install at .kiss/scripts/powershell/ for extension resolution
    specify_ps = tmp_path / ".kiss" / "scripts" / "powershell"
    specify_ps.mkdir(parents=True)
    shutil.copy(common_ps, specify_ps / "common.ps1")
    # Copy extension script
    ext_ps = tmp_path / ".kiss" / "extensions" / "git" / "scripts" / "powershell"
...
class TestGetFeaturePathsSinglePrefix:
    ...
    def test_bash_specify_feature_prefixed_resolves_by_prefix(self, tmp_path: Path):
        """get_feature_paths: KISS_FEATURE with one optional prefix uses effective name for lookup."""
        (tmp_path / ".kiss").mkdir()
        (tmp_path / "specs" / "001-target-spec").mkdir(parents=True)
        cmd = (
            f'cd "{tmp_path}" && export KISS_FEATURE="feat/001-other" && '
    ...
    def test_ps_specify_feature_prefixed_resolves_by_prefix(self, git_repo: Path):
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={**os.environ, "KISS_FEATURE": "feat/001-other"},
        )
        assert result.returncode == 0, result.stderr
        for line in result.stdout.splitlines():
...
class TestGetCurrentBranch:
    def test_env_var(self):
        """Test 12: get_current_branch returns KISS_FEATURE env var."""
        result = source_and_call("get_current_branch", env={"KISS_FEATURE": "my-custom-branch"})
...
def ps_git_repo(tmp_path: Path) -> Path:
    """Create a temp git repo with PowerShell scripts and .kiss dir."""
    subprocess.run(["git", "init", "-q"], cwd=tmp_path, check=True)
    subprocess.run(
        ["git", "config", "user.email", "test@example.com"], cwd=tmp_path, check=True
    )
...
class TestGitBranchNameOverrideBash:
    ...
    def _run_ext(self, ext_git_repo: Path, env_extras: dict, *extra_args: str):
        script = ext_git_repo / ".kiss" / "extensions" / "git" / "scripts" / "bash" / "create-new-feature.sh"
...
class TestGitBranchNameOverridePowerShell:
    ...
    def _run_ext(self, ext_ps_git_repo: Path, env_extras: dict):
        script = ext_ps_git_repo / ".kiss" / "extensions" / "git" / "scripts" / "powershell" / "create-new-feature.ps1"
        return subprocess.run(
...
class TestFeatureDirectoryResolution:
    """Tests for KISS_FEATURE_DIRECTORY and .kiss/feature.json resolution."""

    @requires_bash
    def test_env_var_overrides_branch_lookup(self, git_repo: Path):
        """KISS_FEATURE_DIRECTORY env var takes priority over branch-based lookup."""
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={**os.environ, "KISS_FEATURE_DIRECTORY": str(custom_dir)},
        )
        assert result.returncode == 0, result.stderr
    ...
    def test_feature_json_overrides_branch_lookup(self, git_repo: Path):
        ...
        feature_json = git_repo / ".kiss" / "feature.json"
    ...
    def test_env_var_takes_priority_over_feature_json(self, git_repo: Path):
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={**os.environ, "KISS_FEATURE_DIRECTORY": str(env_dir)},
        )
        assert result.returncode == 0, result.stderr
        for line in result.stdout.splitlines():
            if line.startswith("FEATURE_DIR="):
    ...
    def test_ps_env_var_overrides_branch_lookup(self, git_repo: Path):
        """PowerShell: KISS_FEATURE_DIRECTORY env var takes priority."""
        common_ps = COMMON_PS
