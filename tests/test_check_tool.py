...
Covers issue https://github.com/DauQuangThanh/kiss-u/issues/550:
  `kiss check` reports "Claude Code CLI (not found)" even when claude is
  installed via npm-local (the default `claude` installer path).
"""

from unittest.mock import patch, MagicMock

from kiss_cli.installer import check_tool
...
class TestCheckToolClaude:
    ...
    def test_detected_via_migrate_installer_path(self, tmp_path):
        ...
        with patch("kiss_cli.installer.CLAUDE_LOCAL_PATH",fake_claude), \
             patch("kiss_cli.installer.CLAUDE_NPM_LOCAL_PATH",fake_missing), \
    ...
    def test_detected_via_npm_local_path(self, tmp_path):
        ...
        with patch("kiss_cli.installer.CLAUDE_LOCAL_PATH",fake_migrate), \
             patch("kiss_cli.installer.CLAUDE_NPM_LOCAL_PATH",fake_npm_claude), \
             patch("shutil.which", return_value=None):
            assert check_tool("claude") is True
    ...
    def test_detected_via_path(self, tmp_path):
        ...
        with patch("kiss_cli.installer.CLAUDE_LOCAL_PATH",fake_missing), \
             patch("kiss_cli.installer.CLAUDE_NPM_LOCAL_PATH",fake_missing), \
