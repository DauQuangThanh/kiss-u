...
# Automatically commit changes after a kiss command completes.
...
$configFile = Join-Path $repoRoot ".kiss/extensions/git/git-config.yml"
$enabled = $false
$commitMsg = ""
...
if (-not $commitMsg) {
    $commitMsg = "[kiss] Auto-commit $phase $commandName"
}
