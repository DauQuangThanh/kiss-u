...
#  1. .kiss/scripts/powershell/common.ps1 under the project root
...
if ($projectRoot) {
    $candidates = @(
        (Join-Path $projectRoot ".kiss/scripts/powershell/common.ps1"),
...
if ($Json) {
    ...
    if (-not $DryRun) {
        Write-Output "KISS_FEATURE environment variable set to: $branchName"
    }
}
