...
#  1. .kiss/scripts/bash/common.sh under the project root (installed project)
...
if [ -n "$_PROJECT_ROOT" ] && [ -f "$_PROJECT_ROOT/.kiss/scripts/bash/common.sh" ]; then
    source "$_PROJECT_ROOT/.kiss/scripts/bash/common.sh"
    _common_loaded=true
...
if [ "$DRY_RUN" != true ]; then
    ...
    printf '# To persist: export KISS_FEATURE=%q\n' "$BRANCH_NAME" >&2
fi
...
if $JSON_MODE; then
    ...
    if [ "$DRY_RUN" != true ]; then
        printf '# To persist in your shell: export KISS_FEATURE=%q\n' "$BRANCH_NAME"
    fi
fi
