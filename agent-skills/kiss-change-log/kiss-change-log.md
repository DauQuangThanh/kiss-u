---
name: kiss-change-log
description: >-
  Maintains the change register — one row per applied bug-fix — with
  bug id, commit/PR, files touched, regression test, and the
  reviewer. Records what actually shipped; does not ship anything.
  Use when recording a bug fix that has shipped, updating the change
  log after a release, or tracking which commits fixed which bugs.
license: MIT
