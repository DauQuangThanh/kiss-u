...
name: kiss-plan
description: >-
  Executes the implementation planning workflow using the plan template
  ...
  design documents, or before running kiss-taskify.
license: MIT
...
3. **Agent context update**:
   - Update the plan reference between the `<!-- KISS START -->` and `<!-- KISS END -->` markers in `__CONTEXT_FILE__` to point to the plan file created in step 1 (the IMPL_PLAN path)
