#!/usr/bin/env bash
# Bootstrap a project-local virtualenv for kiss-docx-markdown.
#
...
#
# Note: pandoc is a separate system dependency. Install via your package
# manager (apt, brew, choco) or from https://pandoc.org/installing.html.
set -euo pipefail

SKILL_NAME="kiss-docx-markdown"
...
if ! command -v pandoc > /dev/null 2>&1; then
    echo
    echo "[${SKILL_NAME}] warning: 'pandoc' is not on PATH."
    echo "  This skill uses pandoc for the docx <-> md conversion."
    echo "  Install via:  brew install pandoc  |  apt install pandoc  |  https://pandoc.org/installing.html"
fi
