...
When `kiss workflow run` is invoked, the engine loads a YAML definition, resolves inputs, and dispatches steps sequentially through the step registry:

```mermaid
flowchart TD
    A["kiss workflow run my-workflow"] --> B["WorkflowEngine.load_workflow()"]
...
Steps execute sequentially. Each step receives a `StepContext` containing resolved inputs, accumulated step results, and workflow-level defaults. After execution, the step's output is stored in `context.steps[step_id]` and made available to subsequent steps via expressions like `{{ steps.kiss.output.file }}`.
...
When a `gate` step pauses execution, the engine persists `current_step_index` and all accumulated `step_results`. On `kiss workflow resume <run_id>`, the engine restores the context and continues from the paused step.
...
The engine ships with 10 built-in step types, each in its own subpackage under `src/kiss_cli/workflows/steps/`:

| Type Key | Class | Purpose | Returns `next_steps`? |
|----------|-------|---------|-----------------------|
| `command` | `CommandStep` | Invoke an installed kiss command via integration CLI | No |
...
All step types register into `STEP_REGISTRY` via `_register_builtin_steps()` in `src/kiss_cli/workflows/__init__.py`. The registry maps `type_key` strings to step instances:
...
Workflow definitions use Jinja2-like `{{ expression }}` syntax for dynamic values. The expression engine in `src/kiss_cli/workflows/expressions.py` supports:
...
```mermaid
flowchart TD
    A["kiss workflow search"] --> B["WorkflowCatalog.get_active_catalogs()"]
    B --> C{KISS_WORKFLOW_CATALOG_URL set?}
    C -- Yes --> D["Single custom catalog"]
    C -- No --> E{.kiss/workflow-catalogs.yml exists?}
    E -- Yes --> F["Project-level catalog stack"]
    E -- No --> G{"~/.kiss/workflow-catalogs.yml exists?"}
...
Catalogs are fetched with a 1-hour cache (per-URL, SHA256-hashed cache files in `.kiss/workflows/.cache/`). Each catalog entry has a `priority` (for merge ordering) and `install_allowed` flag.

When `kiss workflow add <id>` installs from catalog, it downloads the workflow YAML from the catalog entry's `url` field into `.kiss/workflows/<id>/workflow.yml`.

## State and Configuration Locations

| Component | Location | Format | Purpose |
|-----------|----------|--------|---------|
| Workflow definitions | `.kiss/workflows/{id}/workflow.yml` | YAML | Installed workflow definitions |
| Workflow registry | `.kiss/workflows/workflow-registry.json` | JSON | Installed workflows metadata |
| Run state | `.kiss/workflows/runs/{run_id}/state.json` | JSON | Persisted execution state |
| Run inputs | `.kiss/workflows/runs/{run_id}/inputs.json` | JSON | Resolved input values |
| Run log | `.kiss/workflows/runs/{run_id}/log.jsonl` | JSONL | Append-only event log |
| Catalog cache | `.kiss/workflows/.cache/*.json` | JSON | Cached catalog entries (1hr TTL) |
| Project catalogs | `.kiss/workflow-catalogs.yml` | YAML | Project-level catalog sources |
| User catalogs | `~/.kiss/workflow-catalogs.yml` | YAML | User-level catalog sources |
...
└── __init__.py              # CLI commands: kiss workflow run/resume/status/
                             #   list/add/remove/search/info,
                             #   kiss workflow catalog list/add/remove
```
