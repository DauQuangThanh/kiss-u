...
function Get-KissEffectiveBranchName {
    param([string]$Branch)
    if ($Branch -match '^([^/]+)/([^/]+)$') {
        return $Matches[2]
    }
...
function Test-FeatureBranch {
    ...
    $raw = $Branch
    $Branch = Get-KissEffectiveBranchName $raw
...
function Find-FeatureDirByPrefix {
    ...
    $specsDir = Join-Path $RepoRoot 'specs'
    $branchName = Get-KissEffectiveBranchName $Branch
...
function Get-FeaturePathsEnv {
    ...
    $featureJson = Join-Path $repoRoot '.kiss/feature.json'
    if ($env:KISS_FEATURE_DIRECTORY) {
        $featureDir = $env:KISS_FEATURE_DIRECTORY
        ...
        try {
            ...
            [Console]::Error.WriteLine("ERROR: Failed to parse .kiss/feature.json: $_")
            exit 1
        }
...
function Resolve-Template {
    ...
    $base = Join-Path $RepoRoot '.kiss/templates'
    ...
    $presetsDir = Join-Path $RepoRoot '.kiss/presets'
    ...
    $extDir = Join-Path $RepoRoot '.kiss/extensions'
    if (Test-Path $extDir) {
